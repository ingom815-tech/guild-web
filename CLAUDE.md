...
- 배포된 프론트 = **v=66** (커밋 표의 마지막 행). 로컬 미푸시 작업 없음.
...
| (최신) | 전력판(전위/중위/후위) 신설 — war_roles.line(0024, null=미지정), members `war_line` 액션·view=war/스냅샷에 line 포함(함수 재배포), 분석·현황탭 [역할 보드\|전력판] 하위 탭 토글, 세로로 쌓인 가로 행(왼쪽 파스텔 밴드: 전위 빨강/중위 주황/후위 파랑/미지정 회색), 칩 클릭→섹터 팝오버(현재 섹터 강조·해제), 짝지는 함께 이동(양쪽 각각 저장), 역할 배치 해제 시 섹터도 소멸 — **현재 배포 버전 (v=66)** |
