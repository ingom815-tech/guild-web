...
- 배포된 프론트 = **v=80** (커밋 표의 마지막 행). 로컬 미푸시 작업 없음.
...
| (최신) | 시즌별 참여 기록 기여점수순 정렬+기여점수 열(합산 참여점수×0.7+전투력×0.3, v=79) + 참여율 관리에 닉네임 변경 이력 카드(`renderNickHistory` — status.nick_history 기반 검색: 이전닉/현재닉/아이디, 결사 열은 listMembers 매핑. 이력은 합병 0016 리셋 이후 내정보 변경분만 — 관리자 대리 변경은 미기록, 합병 전은 `member_nick_history_archive`) — **현재 배포 버전 (v=80)** |
