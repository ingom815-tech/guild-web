...
- 배포된 프론트 = **v=88** (커밋 표의 마지막 행). 로컬 미푸시 작업 없음.
...
| (최신) | 참여 로그 파서: 활동 태그 없는 저장 블록을 **!시틈으로 자동 분류**(defaulted 플래그 — 미리보기에 "태그 없음 → 자동" 배지). 태그 없는 연속 블록은 저장 블록별 분리(splitUntagged), 마지막 태그 뒤 태그 없는 블록도 처리(기존엔 버려짐), 카톡 경로는 태그 못 찾으면 원본 블록만 파싱(명단 오염 방지) — **현재 배포 버전 (v=88)** |
