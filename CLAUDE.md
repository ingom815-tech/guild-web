...
- 배포된 프론트 = **v=68** (커밋 표의 마지막 행). 로컬 미푸시 작업 없음.
...
- **!쟁 지표 (구 !긴급 — 데이/나이트 조 선택 폐지, 0019에서 테이블/컬럼 삭제됨)**: `!쟁` 태그(파서는 레거시 `!긴급`도 쟁으로 매핑)는 점수·참여율과 분리된 별도 지표 — members/season_participation의 `jaeng_count/jaeng_rate/jaeng_morning/jaeng_evening/jaeng_dawn`. 시간대: 오전 09:00~17:00 / 오후 17:01~23:00 / 새벽 23:01~08:59 (log_datetime 기준, RPC가 집계). 대시보드 표는 "일정참여율 | 쟁참여율"(쟁 횟수·시간대는 툴팁), 참여 현황·시즌 기록은 쟁/쟁률 열.
...
| (최신) | 대시보드 참여율 50% 미만 빨간 강조 제거 + 드랍 파서 장비 부위 약칭 별칭 11종(전승서→계율서, 상의→지배자 셔츠, 투구→지배의 머리띠 등 — logParser ITEM_ALIASES) + item_master에 '신화아퀴조각상자' 추가(SQL, 신화/기타) — **현재 배포 버전 (v=68)** |
