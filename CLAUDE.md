...
- 배포된 프론트 = **v=48** (커밋 표의 마지막 행). 로컬 미푸시 작업 없음.
...
- **신화 등급은 분배 신청 제외**: 신청 탭 목록에서 미표시(프론트 필터) + 서버 POST 400 차단. 운영진 수동 분배로 처리(분배 현황/확정 화면에서는 기존대로 취급). 결사 창고에서 신화 타일 클릭 시 "운영진 문의" 토스트.
...
| (최신) | 지망제 v2 화면(4스텝/다크 지망 카드) + 결사 창고(분배 첫 하위탭, 시안 v8) + 신화 분배 제외 — **현재 배포 버전 (v=48)** |
