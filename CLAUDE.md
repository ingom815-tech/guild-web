...
- 배포된 프론트 = **v=90** (커밋 표의 마지막 행). 로컬 미푸시 작업 없음.
...
- **참여점수 계산 로직 (2026-07-30 !쟁 개편판 — 사용자 승인으로 개정)**: 참여점수 = 5종(본토/시틈/유니/결던/별봉)×100, 참여율 분모도 쟁 제외. `recalc_participation_scores` RPC(0018) 공식 외 임의 변경 금지. 점수는 로그에서 재계산되는 파생값 — **직접 UPDATE 금지**(다음 재계산에 씻겨나감).
...
- **!쟁 지표 (구 !긴급 — 데이/나이트 조 선택 폐지, 0019에서 테이블/컬럼 삭제됨)**: `!쟁` 태그(파서는 레거시 `!긴급`도 쟁으로 매핑)는 점수·참여율과 분리된 별도 지표 — members/season_participation의 `jaeng_count/jaeng_rate/jaeng_morning/jaeng_evening/jaeng_dawn`. 시간대: 오전 09:00~17:00 / 오후 17:01~23:00 / 새벽 23:01~08:59 (log_datetime 기준, RPC가 집계). **0025부터 쟁 지표에는 유니/결던 세션도 포함**(쟁 분모 = 쟁+유니+결던 — 일정 점수·참여율은 불변, 유니/결던은 양쪽 모두 반영). 대시보드 표는 "일정참여율 | 쟁참여율"(쟁 횟수·시간대는 툴팁), 참여 현황·시즌 기록은 쟁/쟁률 열.
- **가입 승인 시 출석 소급 매칭(백필)**: members 함수 approve가 현재 시즌 미매칭 참석 행(member_name = 가입 닉네임 정확 일치)을 자동 연결 + 재계산. 닉이 다르면 관리자 참석 보정으로 수동 처리. 지난 시즌은 소급 안 함(스냅샷 보존).
...
| d739474 | 전력 분석/현황 탭 결사별 페이지 전환 — "전체 결사" 통합 뷰 제거, 결사 칩(알파/베타/감마/델타)이 페이지: 보드·미배치 풀·짝지·카운트 전부 현재 결사만, 소속 빈 인원은 "(미지정)" 페이지(저장 차단), 저장은 현재 결사 스냅샷만 발행 (v=62) |
| (최신) | 전력 역할 6종 개편(딜러→마딜러/물딜러, 0023 — 레거시 dealer는 클래스로 정규화: 태양감시자·주문각인사→마딜러) + 보드 레이아웃 대개편: 역할=가로 행+왼쪽 파스텔 소제목 밴드, 행당 칩 6개 고정, 2단 칩(직업 위/닉 아래·5자 말줄임·툴팁), 짝지 번호=칩 우하단 12px 초소형 배지, 메인=우상단 별 배지(남색 음영 제거), 짝지 편성=보드 오른쪽 세로 패널(닉 54px 고정폭), 미배치 풀 240px (members 함수 재배포) (v=63) |
| fb7fc12 | 메인 별 등급 음영: ★3=진한 남색+흰 글씨(.m3), ★2=옅은 남색 #E3E8F0(.m2), ★1=음영 없음(별 배지만) (v=64) |
| 17e9b5b | 상단 "1단계 파일럿" 안내 배너 제거 (초기 배포 잔재) (v=65) |
| (최신) | 전력판(전위/중위/후위) 신설 — war_roles.line(0024, null=미지정), members `war_line` 액션·view=war/스냅샷에 line 포함(함수 재배포), 분석·현황탭 [역할 보드\|전력판] 하위 탭 토글, 세로로 쌓인 가로 행(왼쪽 파스텔 밴드: 전위 빨강/중위 주황/후위 파랑/미지정 회색), 칩 클릭→섹터 팝오버(현재 섹터 강조·해제), 짝지는 함께 이동(양쪽 각각 저장), 역할 배치 해제 시 섹터도 소멸 (v=66) |
| (최신) | 전력판 가독성 개선(결사원 피드백): 섹터 행 전체 색 틴트+진한 밴드+간격 확대, 전력판 칩 단순화(직업+닉+별 배지만 — 음영·짝 번호 배지·금테 제거), 짝지=같은 색 2px 테두리+짝 번호순 나란히 정렬(툴팁에 짝 번호), 현황탭 본인 칩 강조(초록 이중 테두리+"나" 배지, 역할 보드·전력판 공통 — `window.Auth` 참조 버그 수정: const라 typeof 가드) (v=67) |
| 2f6c886 | 대시보드 참여율 50% 미만 빨간 강조 제거 + 드랍 파서 장비 부위 약칭 별칭 11종(전승서→계율서, 상의→지배자 셔츠, 투구→지배의 머리띠 등 — logParser ITEM_ALIASES) + item_master에 '신화아퀴조각상자' 추가(SQL, 신화/기타) (v=68) |
| (최신) | 쟁 지표에 유니/결던 포함(0025 — 일정은 불변, 쟁 분모 = 쟁+유니+결던, RPC만 교체) + 드랍 파서: 별칭 매칭은 확정(유사매칭 확인 불필요), 붙은 수량 분리를 퍼지 매칭보다 우선("신화아퀴조각상자3개"→×3) + 참여율 표시 소수점 반올림(대시보드·KPI·전력분석 풀 카드) (v=69) |
...
| (최신) | 시즌별 참여 기록 필터: 시즌 칩 복수 선택(점수·횟수 합산, 율은 단순 평균) + 결사 칩 복수 선택 + 정렬(참여점수/전투력순) + 결사·전투력 열 추가. 결사·전투력은 listMembers 매핑(프론트만, 함수 무변경). ⚠ 참여 현황 카드에 `#partGuildChips` 기존재 — 시즌 기록용은 `#partSsGuildChips` 별도 id(충돌 사고 수정) (v=76) |
...
| d83289e | **브로치 분배 체크 + 스펙 갱신 알림**: ①분배 하위탭 "브로치" 신설(js/brooch.js) — 전 회원이 기여점수순 명단(순번·닉·결사·전투력·쟁참여율·기여점수) 조회, 운영진만 체크+저장(distribution `view=brooch`/`action=brooch`, app_settings `brooch_distribution` JSON — SQL 불필요, 관리자 제외, 탭 진입마다 재조회, 구버전 함수 응답 시 재배포 안내). ②로그인 시 스펙 갱신 알림 — 장비·아퀴룬 **스샷 각각**의 마지막 업로드 시각(스샷 파일명 13자리 epoch ms)이 기준, 하나라도 20일 경과/기록 없음이면 **매 로그인마다** 모달(스누즈 없음): 전투력 입력(필수)+장비 1장·아퀴 최대 10장 첨부 → 갱신하기(updateProfile+uploadProfileImages, 분배 잠금 중엔 생략). profile 함수는 내정보 저장·인증샷 업로드 시 `profile_updated_at` 스탬프 복원(알람 필수 아님, 기록용). distribution·profile 함수 재배포 완료 (v=84) |
| b420a9c | **수동 분배 등록**: 분배 이력 탭에 운영진 전용 "➕ 수동 분배" 버튼 — 정식 신청 절차 없이 나간 아이템을 이력에 직접 기록(distribution `action=manual_history`, DB 무변경). 품목명(+사유 괄호 병기)·구분·등급·수량·수령자(닉 검색 datalist→user_id 박제, 내 분배이력에도 표시)·판매 대금 다이아(선택 — 룻자 계좌로 공금 입금, dia_account_alias 적용, ref_type manual_distribution). looter="수동 분배"로 정식 분배와 구분, 재고 차감 없음, 분배취소 숨김(삭제만 — 다이아 걸린 건 삭제 시 공금 미복원 경고). 모달 640px 3열 그리드(.modal.wide/.mdgrid). distribution 함수 재배포 완료 (v=85) |
| 83f604b | 시즌별 참여 기록 헤더 정리: 시즌 칩·결사 칩을 각각 "시즌"/"결사" 라벨이 붙은 2줄로 분리(검색·정렬은 제목 줄 우측 유지, 결사 줄 아래 구분선) (v=86) |
| 5727a8c | 참여율 테이블 "쟁률"→"쟁참여율" 명칭 통일(시즌별 기록·참여 현황·합산 안내문) + 시즌별 기록의 쟁참여율 열 강조(.jrcol — 연녹 배경·진녹 볼드) (v=87) |
| a2d1507 | 참여 로그 파서: 활동 태그 없는 저장 블록을 **!시틈으로 자동 분류**(defaulted 플래그 — 미리보기에 "태그 없음 → 자동" 배지). 태그 없는 연속 블록은 저장 블록별 분리(splitUntagged), 마지막 태그 뒤 태그 없는 블록도 처리(기존엔 버려짐), 카톡 경로는 태그 못 찾으면 원본 블록만 파싱(명단 오염 방지) (v=88) |
| 633ac55 | 규정 탭 결사 회의(2026-09-22) 반영: 3단 브로치 확정(캐릭터당 1회·최대 5개, 전투력 상위 순, 우선 조건 ①결사증표 ②태양석 ③전단계 풀강, 전승 간 전투력 차이 미포함) · 영웅 재해 확정(기여점수 순) · 신규 전퀴 지원 개정(라인 4종, 쟁 참여도 우선, 희망자 조사→운영진 회의, PVE/PVP 각 1개 시즌당) · 쟁 참여도 우수 보상 신설(최대 10명, 선택지 4종) · 개정 이력 줄 — 별도로 item_master에 A1/B6 라인 전설 아퀴룬 SQL 등록(드랍 매칭용, C1은 확인 안내) (v=89) |
| (최신) | 시즌별 참여 기록 기본 정렬 = 쟁참여율순(#partSeasonSort selected) + 쟁 우수 보상 카드 인원 문구 명확화("최대 10명 한도 · 10명 고정 지급 아님, 1~2명만 선정될 수도" 강조 줄) — **현재 배포 버전 (v=90)** |
