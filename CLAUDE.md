...
- 배포된 프론트 = **v=63** (커밋 표의 마지막 행). 로컬 미푸시 작업 없음.
...
| (최신) | 전력 역할 6종 개편(딜러→마딜러/물딜러, 0023 — 레거시 dealer는 클래스로 정규화: 태양감시자·주문각인사→마딜러) + 보드 레이아웃 대개편: 역할=가로 행+왼쪽 파스텔 소제목 밴드, 행당 칩 6개 고정, 2단 칩(직업 위/닉 아래·5자 말줄임·툴팁), 짝지 번호=칩 우하단 12px 초소형 배지, 메인=우상단 별 배지(남색 음영 제거), 짝지 편성=보드 오른쪽 세로 패널(닉 54px 고정폭), 미배치 풀 240px (members 함수 재배포) — **현재 배포 버전 (v=63)** |
