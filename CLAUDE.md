...
- 배포된 프론트 = **v=67** (커밋 표의 마지막 행). 로컬 미푸시 작업 없음.
...
| (최신) | 전력판 가독성 개선(결사원 피드백): 섹터 행 전체 색 틴트+진한 밴드+간격 확대, 전력판 칩 단순화(직업+닉+별 배지만 — 음영·짝 번호 배지·금테 제거), 짝지=같은 색 2px 테두리+짝 번호순 나란히 정렬(툴팁에 짝 번호), 현황탭 본인 칩 강조(초록 이중 테두리+"나" 배지, 역할 보드·전력판 공통 — `window.Auth` 참조 버그 수정: const라 typeof 가드) — **현재 배포 버전 (v=67)** |
